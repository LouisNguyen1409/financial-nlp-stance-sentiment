...
\author{
    \IEEEauthorblockN{Louis Nguyen (z5428797) \quad Quoc Dat Bui (z5404752) \quad Nam Khanh Tran (z5577208) \quad Quang Minh Phan (z5531827)}
    \IEEEauthorblockA{COMP6713 Natural Language Processing -- 2026 T1\\
...
% ══════════════════════════════════════════════════════════════════════
%  SCOPE TABLE (required by submission guidelines)
% ══════════════════════════════════════════════════════════════════════

\begin{table*}[t]
\centering
\caption{Project Scope (COMP6713 2026 T1). Minimum 80 credits required; achieved \textbf{155+ credits}.}
\label{tab:scope}
\small
\begin{tabular}{@{}p{3.2cm}p{1.5cm}p{5cm}p{5.5cm}@{}}
\toprule
\textbf{Category} & \textbf{Credits} & \textbf{Description} & \textbf{Our Implementation} \\
\midrule
\multicolumn{4}{@{}l}{\textbf{Part A: Problem Definition} (min. 10 credits) \hfill \textit{Achieved: 20}} \\
NLP Problem (x2) & 5 each & Stance + Sentiment classification & Hawkish/dovish/neutral + Positive/negative/neutral \\
Text Domain (x2) & 5 each & Central bank comms + Financial news & FOMC meeting minutes + Financial PhraseBank headlines \\
\midrule
\multicolumn{4}{@{}l}{\textbf{Part B: Dataset Selection} (min. 20 credits) \hfill \textit{Achieved: 20}} \\
2 existing datasets & 10 & Publicly available & FOMC Hawkish-Dovish (2,480) + Financial PhraseBank (2,264) \\
1 existing lexicon & 10 & Domain lexicon & Loughran-McDonald Financial Sentiment Dictionary \\
\midrule
\multicolumn{4}{@{}l}{\textbf{Part C: Modelling} (min. 30 credits) \hfill \textit{Achieved: 80+}} \\
Baseline & req. & Simple baseline & TF-IDF + Logistic Regression \\
Alternative baselines & 5+ & Additional non-neural models & TF-IDF+SVM (LinearSVC), TF-IDF(trigrams)+LR, TF-IDF+LM Lexicon \\
Pre-trained models (x3) & 5 each & Compare multiple & FinBERT, BERT-base, RoBERTa-base (zero/few-shot) \\
Fine-tune a model & 20 & Out-of-box fine-tuning & FinBERT fine-tuned on each dataset \\
Extend a method & 30 & Novel extension & Multi-task FinBERT (shared encoder + dual heads + weighted CE, alternating batches) \\
Second extension & 15+ & Advanced fine-tuning recipe & BERT-base with Layer-wise LR Decay + Gradual Unfreezing + label smoothing \\
\midrule
\multicolumn{4}{@{}l}{\textbf{Part D: Evaluation} (min. 20 credits) \hfill \textit{Achieved: 35}} \\
Quantitative eval & 10 & Appropriate metrics & Per-class F1, macro-F1, accuracy, confusion matrices \\
Qualitative eval & 5 & Error analysis & Misclassification patterns, ambiguous sentence analysis \\
Command line testing & 5 & CLI interface & \texttt{cli.py} with --text, --file, interactive modes \\
Demo & 10 & Gradio interface & Web UI with stance + sentiment predictions \\
Public model release & 5 & HuggingFace Hub & 5 model repos under \texttt{Louisnguyen/*} via \texttt{push\_to\_hf.py} \\
\bottomrule
\end{tabular}
\end{table*}
...
+ LR/SVM, lexicon rule-based), zero-shot and few-shot pre-trained
...
Malo et al.\ \cite{malo2014good} introduced the Financial PhraseBank,
demonstrating that annotator agreement strongly affects benchmark
quality.  Pre-trained
...
\begin{itemize}[leftmargin=*,nosep]
    \item \textbf{TF-IDF + SVM}: Replaces logistic regression with a
          linear support vector machine (\texttt{LinearSVC},
          $C{=}1.0$), whose hinge-loss objective focuses on the support
          vectors near the decision boundary and typically gives a
          small but consistent lift over logistic regression on
...
FinBERT, BERT-base, and RoBERTa-base \cite{liu2019roberta}.  This setting tests how
...
strategies popularised by ULMFiT~\cite{howard2018ulmfit}:
\begin{itemize}[leftmargin=*,nosep]
...
\end{itemize}
Training runs for 10 epochs; at each epoch one additional layer group
is unfrozen, and the checkpoint with the highest validation macro-F1
...
one for sentiment).  Each head is a single linear layer
(\texttt{nn.Linear(768, 3)}) applied to the \texttt{[CLS]} token
representation, with a shared dropout ($p{=}0.1$) inserted between the
encoder output and the heads.
...
library.  Classical ML baselines use \texttt{scikit-learn}.
Experiments are conducted on a single NVIDIA H200 GPU.  Random seeds
...
Before examining model performance, we conduct a thorough analysis of
the datasets to understand their characteristics and challenges.

\subsection{Class Distribution}

The FOMC dataset exhibits notable class imbalance: \textbf{49.4\%} of
sentences are neutral, with hawkish (24.4\%) and dovish (26.2\%)
forming smaller portions.  The Financial PhraseBank shows a similar
pattern with neutral dominating at 61.4\%, followed by positive (25.2\%)
and negative (13.5\%).  This imbalance motivates our use of weighted
cross-entropy loss during training.

\subsection{Text Length Characteristics}

FOMC sentences are notably longer (mean 30.3 words, max 181) compared
to Financial PhraseBank headlines (mean 22.5 words, max 81).  The
greater length and complexity of FOMC text partly explains the higher
difficulty of stance classification, as the model must integrate
information across longer spans to determine policy stance.

\subsection{Lexicon Coverage Analysis}

We analyse the overlap between the Loughran-McDonald lexicon and each
class.  A key finding is that \textbf{hawkish-category words appear
frequently in neutral FOMC sentences}, not just in hawkish ones.  This
overlap explains why rule-based lexicon classifiers perform poorly on
stance (41.5\% accuracy): the presence of ``inflation'' or ``risk'' does
not reliably indicate a hawkish stance without broader context.

For sentiment, lexicon coverage aligns better with class labels:
positive words concentrate in positive sentences and negative words in
negative sentences, explaining the stronger (though still limited)
lexicon baseline performance on sentiment (69.3\%).

\subsection{Discriminative Features}

TF-IDF analysis reveals the most discriminative words per class.  For
stance, hawkish sentences feature terms like ``inflation'',
``tightening'', and ``elevated'', while dovish sentences contain
``accommodation'', ``stimulus'', and ``subdued''.  Critically, many
discriminative terms for stance are domain-specific and absent from
general-purpose models' pre-training data, highlighting the value of
FinBERT's financial pre-training.

For sentiment, discriminative features are more intuitive: ``profit'',
``growth'' for positive; ``loss'', ``decline'' for negative.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{../analysis/performance_progression.png}
    \caption{Performance progression from baselines through multi-task
    learning.  The gap between sentiment (blue) and stance (red) persists
...
\begin{figure}[t]
    ...
    \caption{Few-shot (k=16) comparison showing the advantage of
    domain-specific pre-training.  FinBERT dramatically outperforms
    BERT-base and RoBERTa on sentiment.}
...
Figure~\ref{fig:progression} illustrates this progression across all
modelling stages, while Figure~\ref{fig:domain-gap} quantifies the
domain pre-training advantage in the few-shot setting.
...
domain-specific pre-training.  On sentiment, BERT-base LLRD reaches
96.91\% accuracy, slightly edging out FinBERT fine-tuned (96.69\%)
and confirming that the training regime matters as much as the
starting checkpoint for well-aligned tasks.

\subsection{Traditional Baselines}

Among traditional methods, TF-IDF + SVM is the strongest, achieving
89.40\% on sentiment and 63.31\% on stance.  The linear SVM's
hinge-loss objective concentrates on support vectors near the decision
boundary, giving it a consistent edge over logistic regression on the
sparse high-dimensional TF-IDF feature space.  Extending to trigrams
provides marginal improvement, while adding Loughran-McDonald lexicon
features actually \emph{hurts} TF-IDF performance, likely because the
lexicon introduces noise when its categories do not align perfectly
with the task labels.
...
\subsubsection{Stance Errors: Neutral--Leaning Confusion}
The top-20 stance errors dumped to
\texttt{results/all\_results\_summary.json} are dominated by
confusion between neutral and one of the leaning classes.
Representative examples (truncated as stored in the error log)
include:

\begin{itemize}[leftmargin=*,nosep]
    \item \emph{``With an increase in the target range at this
          m\dots{}''} --- true \emph{hawkish}, predicted
          \emph{neutral}.  The sentence reports a policy action in
          factual language, so the hawkish implication is structural
          rather than lexical.
    \item \emph{``In light of increased uncertainties and muted
          \dots{}''} --- true \emph{dovish}, predicted
          \emph{hawkish}.  The model over-weights ``uncertainties''
          while the following ``muted inflation pressures'' is the
          actual dovish cue.
    \item \emph{``Looking ahead, reports from retailer contacts
          \dots{}''} --- true \emph{neutral}, predicted \emph{dovish}.
          The sentence introduces a data-gathering pass, with no
          real stance.
\end{itemize}

These errors are difficult to resolve without richer contextual
modelling (e.g., document-level features or discourse structure).

\subsubsection{Sentiment Errors}
Only 7 sentiment errors remain on a 453-sentence test set.  Among
them, the most common pattern is \emph{positive}~$\to$~\emph{negative}
on comparative profit/loss statements that contain both positive and
negative cues (e.g., ``\emph{Net profit was 35.5 mln compared with
29.8 mln}'' --- an unambiguous improvement that the model reads as a
loss signal).  A secondary pattern is \emph{neutral}~$\to$~\emph{positive}
on business-expansion announcements where the model over-reads
optimism from terms like ``transferred'' or ``deliver.''  The full
list of seven sentences is included in the error log.
