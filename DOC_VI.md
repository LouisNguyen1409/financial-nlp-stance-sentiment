...
| C: Mô Hình Hóa | 30 | Cơ sở + 3 mô hình sẵn + tinh chỉnh + đa nhiệm | 65+ |
| D: Đánh Giá | 20 | Định lượng + định tính + CLI + demo | 30 |
| **Tổng** | **80** | | **135+** |
...
│   ├── finbert_stance/        # Mô hình lập trường đã tinh chỉnh
│   ├── finbert_sentiment/     # Mô hình cảm xúc đã tinh chỉnh
│   └── multitask_finbert/     # Mô hình đa nhiệm
...
run_experiments.py
    ...
    │
    ├── Bước 2: baseline.py    → TF-IDF + LR trên cả hai bài toán
    │           lexicon.py     → Phân loại dựa trên từ điển LM
    │
    ├── Bước 3: pretrained_eval.py → FinBERT zero-shot
    │                                 Few-shot (FinBERT, BERT, RoBERTa)
    │
    ├── Bước 4: finetune.py    → FinBERT tinh chỉnh trên FOMC
...
### 9.2 Vòng Lặp Huấn Luyện (`src/finetune.py`)
...
| Bài toán | Accuracy | Macro-F1 |
|----------|----------|----------|
| Lập trường | 0.6371 | 0.6194 |
| Cảm xúc | 0.9669 | 0.9459 |

Tinh chỉnh cải thiện so với baseline:
- Lập trường: +2.8% accuracy, +3.2% F1 (khiêm tốn — lập trường là bài toán khó)
- Cảm xúc: +9.5% accuracy, +12.3% F1 (cải thiện đáng kể)
...
|----------|-------------|-------------|-----------|
| Lập trường | 0.6194 | **0.6478** | +0.0284 |
| Cảm xúc | 0.9459 | **0.9666** | +0.0207 |

**Đa nhiệm cải thiện cả hai bài toán.** Bộ mã hóa dùng chung được hưởng lợi
từ việc nhìn thấy văn bản tài chính đa dạng hơn. Bài toán lập trường đặc biệt
được hưởng lợi vì nó có ít mẫu huấn luyện hơn — dữ liệu cảm xúc cung cấp
chính quy hóa hữu ích.

### 10.6 Phân Tích Lỗi

Mô hình đa nhiệm mắc 169 lỗi trên lập trường:
```
trung lập → diều hâu: 49    (phổ biến nhất — câu trung lập có dấu hiệu diều hâu nhẹ)
trung lập → bồ câu:   39    (câu trung lập có dấu hiệu bồ câu nhẹ)
bồ câu → diều hâu:    26    (nhầm bồ câu/diều hâu — khác biệt ngôn ngữ tinh tế)
```
...
Về cảm xúc, chỉ có 9 lỗi tổng cộng — 4 trong đó là "tích cực → tiêu cực"
(ví dụ: câu tích cực về một công ty đề cập đến thua lỗ của đối thủ).
...
| FinBERT (tinh chỉnh) | 0.6371 | 0.6194 | 0.9669 | 0.9459 |
| **FinBERT Đa nhiệm** | **0.6593** | **0.6478** | **0.9801** | **0.9666** |

### 14.2 Các Quan Sát Chính

**1. Huấn luyện sẵn theo miền là yếu tố quan trọng nhất.**
FinBERT (few-shot, chỉ 48 mẫu) đạt 96.7% F1 cho cảm xúc, đánh bại BERT-base
(65.0%) và RoBERTa-base (67.2%) với cùng dữ liệu. Đó là chênh lệch hơn 30
điểm phần trăm chỉ từ huấn luyện sẵn theo miền.

**2. Lập trường về cơ bản khó hơn cảm xúc.**
Ngay cả mô hình tốt nhất cũng chỉ đạt 64.8% F1 cho lập trường so với 96.7%
...
**3. Học đa nhiệm giúp cả hai bài toán.**
Mô hình đa nhiệm đánh bại tinh chỉnh đơn nhiệm trên cả hai bài toán:
- Lập trường: +2.8% F1 (0.6194 → 0.6478)
- Cảm xúc: +2.1% F1 (0.9459 → 0.9666)

Điều này xác nhận lập trường và cảm xúc tài chính là các bài toán liên quan
có lợi từ biểu diễn dùng chung.

**4. Baseline TF-IDF mạnh đáng ngạc nhiên cho cảm xúc.**
87.2% accuracy với mô hình túi từ cho thấy cảm xúc tài chính thường quy về
sự hiện diện từ khóa. Mô hình nơ-ron thêm giá trị chủ yếu cho các trường
hợp mơ hồ.

**5. FinBERT zero-shot xuất sắc cho cảm xúc nhưng thất bại cho lập trường.**
...
**6. Few-shot BERT/RoBERTa hoạt động kém hơn baseline TF-IDF.**
Với chỉ 48 mẫu huấn luyện, embedding transformer đóng băng không nắm bắt đủ
tín hiệu đặc thù bài toán. Baseline TF-IDF, huấn luyện trên 1700+ mẫu, thắng
dễ dàng. Điều này cho thấy nhiều dữ liệu huấn luyện hơn có thể bù đắp cho
...
```
Chỉ quy tắc (từ điển)           → ~40% F1 (không có khả năng học)
Baseline thống kê (LR)           → ~59% F1 (học từ dữ liệu)
Mô hình miền, ít dữ liệu        → ~45% F1 (đúng mô hình, quá ít dữ liệu)
Mô hình miền, đủ dữ liệu        → ~62% F1 (đúng mô hình, đúng dữ liệu)
Mô hình miền đa nhiệm           → ~65% F1 (đúng mô hình, nhiều dữ liệu, học chia sẻ)
```

Mỗi bước thêm một thứ: khả năng học → kiến thức miền → thêm dữ liệu → tín
hiệu đa nhiệm chia sẻ.
...
1. **Luôn bắt đầu với baseline.** TF-IDF + LR mất 5 giây để huấn luyện và
   cho biết bài toán khó như thế nào.

2. **Huấn luyện sẵn theo miền > kích thước mô hình.** FinBERT (110 triệu
   tham số) trên bài toán tài chính đánh bại BERT/RoBERTa đa năng (110-125
   triệu tham số). Hãy dùng mô hình đặc thù miền khi có sẵn.

3. **Học đa nhiệm là hiệu suất miễn phí.** Nếu bạn có các bài toán liên quan,
   huấn luyện chúng đồng thời không tốn thêm chi phí và cải thiện cả hai.

4. **Mất cân bằng lớp phải được xử lý.** Nếu không có mất mát có trọng số,
   mô hình lập trường dự đoán "trung lập" cho hầu hết đầu vào (dễ đạt 49%
   accuracy, khó đạt F1 cao).

5. **Chỉ số đánh giá quan trọng.** Accuracy có thể gây hiểu lầm với lớp
...
### 16.7 `src/finetune.py` — Tinh Chỉnh FinBERT
...
### 16.8 `src/multitask.py` — Học Đa Nhiệm
...
### 16.9 `run_experiments.py` — Điều Phối Thí Nghiệm
...
### 16.10 `cli.py` — Giao Diện Dòng Lệnh
...
*Tài liệu được tạo cho COMP6713 2026 T1. Tất cả code, thí nghiệm, và phân
tích đều có thể tái tạo bằng `python run_experiments.py`.*
