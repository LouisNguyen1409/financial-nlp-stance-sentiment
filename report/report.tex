...
Alternative baselines & 5+ & Additional non-neural models & TF-IDF+SVM (LinearSVC), TF-IDF(trigrams)+LR, TF-IDF+LM Lexicon, GloVe+LR/SVM \\
...
\begin{itemize}[leftmargin=*,nosep]
    ...
    \item \textbf{GloVe + LR / SVM}: Replaces TF-IDF with 100-dimensional
          GloVe \cite{pennington2014glove} embeddings
          (\texttt{glove.6B.100d}) averaged over tokens, providing
          semantically richer but lower-dimensional representations.
          GloVe was produced by a separate local script (run with the
          pre-trained embedding file \texttt{data/glove.6B.100d.txt},
          which is gitignored due to size); its metrics in
          Tables~\ref{tab:sentiment} and~\ref{tab:stance} are loaded
          from the committed JSON artefacts in
          \texttt{results/glove\_*.json}.
\end{itemize}
...
embeddings are 100-dimensional vectors from the Wikipedia + Gigaword
6B corpus (\texttt{glove.6B.100d}).  Experiments are conducted on a
single NVIDIA H200 GPU.  Random seeds are fixed for reproducibility.
