"""
Fine-tuning BERT-base-uncased with Layer-wise Learning Rate Decay (LLRD)
and Gradual Unfreezing.

This is a deliberately different fine-tuning strategy compared to the standard
full fine-tuning of FinBERT used in the other branch. Key differences:

  1. Base model     : bert-base-uncased (general-purpose, not domain-specific)
                      vs ProsusAI/finbert in the other branch.

  2. Gradual         Each epoch, one additional transformer layer group is
     Unfreezing    : unfrozen, starting from the classification head and
                      moving downward to the embeddings. This prevents the
                      catastrophic forgetting that can occur when random
                      gradients from an untrained head immediately corrupt
                      pre-trained representations in lower layers.

  3. Layer-wise      Only the currently unfrozen layers receive gradients.
     LR Decay     : Among them, each layer's learning rate is scaled by a
     (LLRD)          constant factor (default 0.9) relative to the layer
                      above it. The head sees the full base_lr; each
                      successive lower layer is multiplied by 0.9^depth.
                      This preserves stable low-level representations while
                      allowing high-level layers to adapt more aggressively.

  4. Label           CrossEntropyLoss with ε = 0.1 reduces over-confidence
     Smoothing    :  on the relatively small training sets and acts as a
                      light regulariser.

Summary of design choices vs the other branch:
  ┌─────────────────────┬──────────────────────┬──────────────────────────┐
  │                     │ Other branch         │ This branch              │
  ├─────────────────────┼──────────────────────┼──────────────────────────┤
  │ Base model          │ ProsusAI/finbert     │ bert-base-uncased        │
  │ Fine-tuning scheme  │ All layers from ep.1 │ Gradual unfreezing       │
  │ Per-layer LR        │ Uniform 2e-5         │ Decayed by 0.9 per depth │
  │ Loss                │ Standard CE          │ Label-smoothed CE (ε=.1) │
  │ Warmup              │ Linear warmup sched. │ Gradual unfreezing acts  │
  │                     │                      │ as implicit warmup       │
  └─────────────────────┴──────────────────────┴──────────────────────────┘

References:
  Howard, J. and Ruder, S. (2018). "Universal Language Model Fine-Tuning
    for Text Classification." ACL 2018.  (discriminative fine-tuning / ULMFiT)
  Sun, C. et al. (2019). "How to Fine-Tune BERT for Text Classification?"
    CCL 2019.  (gradual unfreezing on BERT)
"""
