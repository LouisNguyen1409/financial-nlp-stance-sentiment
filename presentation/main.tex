...
\begin{frame}{Dataset Overview}
    \centering
    {\footnotesize
    \begin{tabular}{@{}llcp{4.4cm}p{3.8cm}@{}}
        \toprule
    ...
    \begin{itemize}\setlength{\itemsep}{1pt}
        ...
        \item Both are \textbf{imbalanced}, both have \textbf{dominant neutral}, both are \textbf{small by modern NLP standards}.
    \end{itemize}
\end{frame}
...
\begin{frame}{Multi-task FinBERT Architecture}
    \begin{columns}[T]
        ...
        \begin{column}{0.42\textwidth}
            ...
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item \textbf{Alternating batches:} one FOMC batch, then one FPB batch; each updates only its own head plus the shared encoder.
...
\begin{frame}{Baseline \& Lexicon Results}
    ...
    \begin{itemize}\setlength{\itemsep}{1pt}
        ...
        \item \textbf{Trigrams give only marginal lift} over bigrams ($+0.5$ pp accuracy) --- vocabulary grows faster than signal.
...
\begin{frame}{Fine-tuned Single-Task Models}
    ...
    \vspace{0.6em}
    \begin{itemize}\setlength{\itemsep}{0.3em}
        \item \textbf{BERT-base with LLRD + gradual unfreeze matches FinBERT on sentiment} (macro-F1 tied at 0.9670) and \textbf{exceeds it on stance} by $+1.89$ pp F1.
...
\begin{frame}{Key Findings}
    \begin{enumerate}\setlength{\itemsep}{0.35em}
        ...
        \item \textbf{A careful fine-tuning recipe rivals domain pretraining.} BERT-base with LLRD + gradual unfreeze + label smoothing \textit{matches} FinBERT on sentiment and \textit{beats} it on stance~\cite{sun2019finetune,howard2018ulmfit}.
...
\begin{frame}{Deployment --- CLI, Demo, and Hub Release}
    \begin{columns}[T]
        ...
        \begin{column}{0.42\textwidth}
            ...
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item \texttt{Louisnguyen/finbert-fomc-stance}
                \item \texttt{Louisnguyen/finbert-fpb-sentiment}
                \item \texttt{Louisnguyen/bert-llrd-fomc-stance}
                \item \texttt{Louisnguyen/bert-llrd-fpb-sentiment}
                \item \texttt{Louisnguyen/finbert-multitask-fomc-fpb}
            \end{itemize}
